...
<div class="container">
    ...
    <table border="1">
        ...
        // Assurez-vous que l'identifiant de l'utilisateur est stocké dans $_SESSION['userId'] lors de la connexion
        $userId = isset($_SESSION['Id']) ? $_SESSION['Id'] : die("Veuillez vous connecter pour voir les interventions.");

        // Modifier la requête SQL pour filtrer les interventions par l'identifiant de l'utilisateur connecté
        $sql = "SELECT Identifiant, date, client, statut, `type d'intervention`, adresse, `code postal`, `numero tel`, `degré d'urgence` FROM intervention WHERE userId = ?";
